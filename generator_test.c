...
#define FAULT_RF_OVERRUN     (1U << 5U)
#define FAULT_IWDG           (1U << 6U)
#define PIN_POWER            0U  
...
static          rf_detector_t g_rf;
static          button_t   g_button;
static          uint32_t   g_oled_ok          = 0U;

static const rf_config_t g_rf_cfg = {
    JAM_THRESHOLD,
    LOW_THRESHOLD,
    CONFIRM_WINDOWS
};
...
static void gpio_init(void) {
    ...
    REG(GPIOB_CRL) = (REG(GPIOB_CRL) & ~(GPIO_NIBBLE_MASK << (PIN_RF_IN * 4U)))
                   | (GPIO_INPUT_PUPD << (PIN_RF_IN * 4U));
    GPIO_CLR(GPIOB_BSRR, PIN_RF_IN);
...
void TIM3_IRQHandler(void) {
    ...
    rf_push_sample(&g_rf, (REG(GPIOB_IDR) >> PIN_RF_IN) & 1U);
}

static void rf_reset(void) {
    irq_disable();
    g_rf.sample_count     = 0U;
    g_rf.high_count       = 0U;
    g_rf.ready_high_count = 0U;
    g_rf.ready            = 0U;
    g_rf.overrun          = 0U;
    g_rf.state            = RF_UNKNOWN;
    g_rf.candidate        = RF_UNKNOWN;
    g_rf.candidate_count  = 0U;
    g_rf.last_high_count  = 0U;
    irq_enable();
}

static rf_state_t classify(uint16_t high, const rf_config_t *cfg) {
    if (high >= cfg->jam_threshold) { return RF_JAMMING;   }
    if (high <= cfg->low_threshold) { return RF_NO_SIGNAL; }
    return RF_NORMAL;
}

static uint32_t rf_process(const rf_config_t *cfg) {
    uint16_t   high;
    uint8_t    had_overrun;
    rf_state_t observed;

    if (g_rf.ready == 0U) { return 0U; }

    irq_disable();
    high          = g_rf.ready_high_count;
    g_rf.ready    = 0U;
    had_overrun   = g_rf.overrun;
    g_rf.overrun  = 0U;
    irq_enable();

    if (had_overrun != 0U) { g_system_faults |= FAULT_RF_OVERRUN; }

    observed = classify(high, cfg);
    g_rf.last_high_count = high;

    if (observed == g_rf.candidate) {
        if (g_rf.candidate_count < cfg->confirm_windows) {
            g_rf.candidate_count++;
        }
    } else {
        g_rf.candidate       = observed;
        g_rf.candidate_count = 1U;
    }

    if ((g_rf.candidate_count >= cfg->confirm_windows) &&
        (g_rf.state != g_rf.candidate)) {
        g_rf.state = g_rf.candidate;
        return 1U;
    }
    return 0U;
...
static int32_t i2c_write(uint8_t addr, uint8_t ctrl,
                         const uint8_t *data, uint32_t n,
                         uint32_t data_repeated) {
    int32_t  rc;
    uint32_t started = 0U;
...
static int32_t oled_write_checked(uint8_t ctrl,
                                      const uint8_t *data,
                                      uint32_t n,
                                      uint32_t repeated) {
    int32_t rc;
...
static void oled_print(uint32_t page, uint32_t col, const char *s) {
    ...
    while ((*s != '\0') && ((col + OLED_CHAR_WIDTH) <= OLED_WIDTH)) {
        ...
        const uint32_t idx  = ((sidx < 0) || (sidx >= (int32_t)FONT_SIZE))
                              ? 0U : (uint32_t)sidx;
        uint32_t i;
...
static const char *state_text(rf_state_t s) {
    ...
    case RF_NO_SIGNAL: t = "FAULT     "; break;
...
int main(void) {
    ...
    for (;;) {
        ...
        state_changed = (g_powered != 0U) ? rf_process(&g_rf_cfg) : 0U;
